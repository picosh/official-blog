...
We recently enabled the ability for automatic site usage analytics[^1] for tuns sites. With site usage analytics, we provide webmasters with the ability to see unique visitors, referers, top urls, and top 404 urls. It leverages the same analytics system we have for pages[^2] and prose[^3].
...
I wanted to touch briefly on our implementation since it relates to another pico server, pipe[^4], which is our authenticated, networked `*nix` pipes service.
